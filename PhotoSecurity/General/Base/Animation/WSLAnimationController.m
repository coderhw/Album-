...
#import "HHTransitionInteractive.h"

@interface WSLAnimationController () <UINavigationControllerDelegate>

//动画过渡转场
@property (nonatomic, strong) HHTransitionAnimation * transitionAnimation;
//手势过渡转场
@property (nonatomic, strong) HHTransitionInteractive * transitionInteractive;

@end

@implementation WSLAnimationController
...
- (UIImageView *)imageView{
    ...
    if (_imageView == nil) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, APP_WIDTH, APP_HEIGTH/2)];
        _imageView.center = CGPointMake(APP_WIDTH/2, APP_HEIGTH/2);
...
- (UILabel *)titleLabel{
    ...
    if (_titleLabel == nil) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,APP_HEIGTH - StatusBarAndNavigationBarHeight - 30, APP_WIDTH, 40)];
...
- (HHTransitionAnimation *)transitionAnimation {
    
    if (_transitionAnimation == nil) {
        _transitionAnimation = [[HHTransitionAnimation alloc] init];
    }
    return _transitionAnimation;
}

- (HHTransitionInteractive *)transitionInteractive{
    
    if (_transitionInteractive == nil) {
        _transitionInteractive = [[HHTransitionInteractive alloc] init];
